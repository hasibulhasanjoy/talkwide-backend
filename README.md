...
- **SMTP Server** (Gmail, Mailtrap, or custom SMTP for transactional emails)
- **Google Cloud Console** (for Google OAuth client credentials)
...
# Database
MONGO_URI=mongodb://localhost:27017/talkwide
...
git clone https://github.com/hasibulhasanjoy/talkwide-backend.git
cd talkwide-backend

# 2. Install dependencies
npm install

# 3. Start development server with live reload (Nodemon + ts-node)
npm run dev
...
# 5. Build for production (compiles TypeScript to dist/)
npm run build
...
talkwide/
├── src/
│   ├── config/             # Database, Cloudinary, Nodemailer, Passport setups
│   │   ├── cloudinary.config.ts
│   │   ├── db.ts
│   │   ├── nodemailer.config.ts
│   │   └── passport.config.ts
│   ├── controllers/        # HTTP controllers (auth, post, oauth)
│   │   ├── auth.controller.ts
│   │   ├── oauth.controller.ts
│   │   └── post.controller.ts
│   ├── interfaces/         # TypeScript type & document interfaces
│   │   ├── mail.interface.ts
│   │   ├── post.interface.ts
...
│   │   └── express.d.ts
│   ├── utils/              # AppError class, async wrapper, email templates
│   │   ├── appError.class.ts
│   │   ├── asyncErrorHandler.utils.ts
│   │   ├── emailTemplates.util.ts
│   │   └── verifyToken.util.ts
│   ├── app.ts              # Express application setup
│   └── server.ts           # HTTP server initialization & DB connection
...
- [ ] **Threaded Comments Module**: Recursive nested comments with Reddit-style voting and comment karma.
- [ ] **Community / Subreddit Sub-Systems**: Sub-community creation, role-based moderators, custom rules, and community-scoped feeds.
- [ ] **Real-Time Notifications**: WebSocket / Socket.IO integration for instant upvote, comment reply, and mention alerts.
- [ ] **Rate Limiting & Anti-Spam**: Redis-backed rate limiting for auth and post creation endpoints.
- [ ] **Comprehensive Test Suite**: Automated unit and integration testing suite with Vitest / Jest and Supertest.
